...
async function handler() {
  try {
    const script = await redis.json.get<PodcastScript>("ru:generated-script");
    ...
    for (const [index, segment] of script.entries()) {
      await queue.enqueueJSON({
        url: absoluteUrl(`/api/podcast/ru/text-to-speech/base-script/${index}`),
        body: {
          ...segment,
          index,
        },
    ...
    await queue.enqueueJSON({
      url: absoluteUrl("/api/podcast/ru/merge-audio/base-script"),
    });
