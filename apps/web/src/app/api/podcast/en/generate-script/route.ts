...
async function handler() {
  try {
    const feed = await redis.json.get<FeedItem[]>("en:feed-items");
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: GENERATE_SCRIPT_SYSTEM_PROMPT_EN,
        },
        {
          role: "user",
    ...
    await redis.json.set("en:generated-script", "$", script);
    await queue.enqueueJSON({
      url: absoluteUrl("/api/podcast/en/text-to-speech/base-script"),
    });
